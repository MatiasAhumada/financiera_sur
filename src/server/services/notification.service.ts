...
export async function notifyInquiry(inquiry: { id: string; name: string; email: string; phone: string; message: string | null; product?: { name: string } | null }) {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.EMAIL_FROM ?? "Financiera del Sur <onboarding@resend.dev>", to: [recipient], subject: `Nueva consulta de ${inquiry.name}`, html: `<h2>Nueva consulta</h2><p><b>Nombre:</b> ${inquiry.name}</p><p><b>Email:</b> ${inquiry.email}</p><p><b>Teléfono:</b> ${inquiry.phone}</p><p><b>Producto:</b> ${inquiry.product?.name ?? "General"}</p><p>${inquiry.message ?? "Sin mensaje"}</p>` }) });
