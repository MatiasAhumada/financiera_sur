...
export async function getSession(request: Request) { const token = request.headers.get("cookie")?.match(/ds_session=([^;]+)/)?.[1]; if (!token) return null; const session = await prisma.adminSession.findUnique({ where: { tokenHash: hashToken(token) }, include: { user: true } }); if (!session || session.expiresAt < new Date() || !session.user.active) return null; return session; }
