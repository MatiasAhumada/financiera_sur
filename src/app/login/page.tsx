...
import { LOGIN_REDIRECT_DELAY_MS } from "@/constants/admin.constant";
...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await sessionClientService.create({ email: email.trim(), password });
      toastSuccess("Sesión iniciada", { description: "Redirigiendo al panel administrativo." });
      window.setTimeout(() => window.location.assign(ROUTES.ADMIN), LOGIN_REDIRECT_DELAY_MS);
    } catch { const message = "El email o la contraseña no son correctos."; setError(message); toastError(message); } finally { setLoading(false); }
  }
